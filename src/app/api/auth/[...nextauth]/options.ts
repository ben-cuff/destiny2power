    ...
    BungieProvider({
      ...
      profile(profile, tokens) {
        ...
        const primaryMembership = memberships.find(
          (m: { membershipId: any; }) => m.membershipId === membershipId
        );
      
        const profileImage = profile?.Response?.bungieNetUser?.profilePicturePath;
      
        const user: User = {
          id: membershipId,
          name: profile?.Response?.bungieNetUser?.displayName || "Bungie User",
          email: null,
          image: profileImage ? `https://www.bungie.net${profileImage}` : null,
